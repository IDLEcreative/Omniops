 ...
 * the expected accuracy improvements now that the SQL migration is applied.
 * 
...
const TEST_DOMAIN_ID = '8dccd788-1ec1-43c2-af56-78aa3366bad3'; // From DB
...
interface TestResult {
  ...
  highConfidenceChunks: number; // > 0.85
  mediumConfidenceChunks: number; // 0.7-0.85
  lowConfidenceChunks: number; // < 0.7
...
const TEST_QUERIES: TestQuery[] = [
  ...
  {
    id: 'T004',
    query: 'alternator pulley dimensions and compatibility',
    type: 'technical',
    expectedTerms: ['alternator', 'pulley', 'dimensions', 'compatibility'],
    minimumChunks: 10,
    minimumSimilarity: 0.70,
    description: 'Dimensional and compatibility specifications'
  },
  ...
  {
    id: 'C003',
    query: 'Compare torque wrench accuracy across different brands',
    type: 'comparison',
    expectedTerms: ['compare', 'torque', 'wrench', 'accuracy', 'brands'],
    minimumChunks: 15,
    minimumSimilarity: 0.65,
    description: 'Brand comparison for specific feature'
  },
  ...
  {
    id: 'X003',
    query: 'Can you help me choose the right torque wrench for automotive work, including accuracy requirements and price range?',
    type: 'complex',
    expectedTerms: ['torque', 'wrench', 'automotive', 'accuracy', 'requirements', 'price'],
    minimumChunks: 12,
    minimumSimilarity: 0.65,
    description: 'Product selection with multiple criteria'
  }
];
...
class AccuracyTester {
  ...
  async initialize(): Promise<void> {
    console.log('🔧 Initializing Accuracy Test Suite...');
    ...
    console.log(`✅ Test domain verified: ${domainData.domain}`);
  }
  ...
  private async testSingleQuery(testQuery: TestQuery): Promise<TestResult> {
    ...
    // Get enhanced context using the new system
    const enhancedContext = await getEnhancedChatContext(
      ...
      TEST_DOMAIN_ID,
      {
        ...
        minChunks: testQuery.minimumChunks,
        maxChunks: 15
      }
    );
    ...
    const chunks = enhancedContext.chunks;
    ...
    const highConfidenceChunks = chunks.filter(c => c.similarity > 0.85).length;
    const mediumConfidenceChunks = chunks.filter(c => c.similarity > 0.7 && c.similarity <= 0.85).length;
    const lowConfidenceChunks = chunks.filter(c => c.similarity <= 0.7).length;
    ...
    const metadataEnhanced = chunks.some(c => 
      (c as any).source === 'embedding' || 
      (c as any).metadata || 
      enhancedContext.contextSummary
    );
    
    // Identify search sources used
    const searchSources = [...new Set(chunks.map(c => (c as any).source || 'unknown'))];
  ...
  private analyzePositionDistribution(chunks: any[]): number[] {
    ...
    const positions = chunks
      .map(c => c.chunk_index || c.position || 0)
  ...
  private generateRecommendations(): string[] {
    ...
    const failedCategories = ['product', 'technical', 'comparison', 'complex'].filter(category => {
      const categoryTests = this.results.filter(r => r.type === category);
      const categoryAccuracy = categoryTests.filter(r => r.passed).length / categoryTests.length;
      return categoryAccuracy < (ACCURACY_TARGETS as any)[category];
    });
    
    if (failedCategories.length > 0) {
      recommendations.push(`Categories needing improvement: ${failedCategories.join(', ')}. Consider specialized optimization for these query types.`);
    }
    
    // High confidence recommendations
    const avgHighConfidence = this.results.reduce((sum, r) => sum + r.highConfidenceChunks, 0) / this.results.reduce((sum, r) => sum + r.chunksRetrieved, 0);
    if (avgHighConfidence < ACCURACY_TARGETS.highConfidenceRate) {
      recommendations.push(`High confidence chunk rate ${(avgHighConfidence * 100).toFixed(1)}% is below target ${(ACCURACY_TARGETS.highConfidenceRate * 100)}%. Consider improving embedding quality or content enrichment.`);
    }
...
class PerformanceMonitor {
  static async measureTokenUsage(query: string): Promise<{ embeddingTokens: number; promptTokens: number }> {
    // Estimate token usage for embeddings and prompts
    const embeddingTokens = Math.ceil(query.length / 4); // Rough estimate: 1 token ≈ 4 characters
    const promptTokens = 0; // Would need to integrate with actual chat completion
    
    return { embeddingTokens, promptTokens };
  }
  
  static async measureCacheHitRate(): Promise<number> {
    // Would need to integrate with actual cache implementation
...
// Main execution
async function main() {
  console.log('🔍 Enhanced Context Window Accuracy Validation');
  ...
  try {
    ...
    const reportPath = '/Users/jamesguy/Omniops/accuracy-test-results.json';
...
export { AccuracyTester, PerformanceMonitor, type TestResult, type AccuracyReport };
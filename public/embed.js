(async function() {
  ...
  async function fetchFromCandidates(candidates, path, fetchOptions, parser) {
    ...
    for (const origin of candidates) {
      try {
        const response = await fetch(`${origin}${path}`, fetchOptions);
        if (!response.ok) {
          lastError = new Error(`Request failed with status ${response.status}`);
          continue;
        }

        const parsedResponse = parser ? await parser(response) : response;
        return { data: parsedResponse, origin };
      } catch (error) {
        lastError = error;
      }
    }
    ...
    throw new Error('Request failed for all server URL candidates');
  }
  ...
  try {
    ...
    if (!userConfig.skipRemoteConfig) {
      try {
        const serverCandidates = createServerUrlCandidates(config.serverUrl);
        const searchParams = new URLSearchParams({ domain: currentDomain });
        const { data: remoteResponse, origin: resolvedOrigin } = await fetchFromCandidates(
          serverCandidates,
          `/api/widget/config?${searchParams.toString()}`,
          undefined,
          response => response.json()
        );

        if (remoteResponse.success && remoteResponse.config) {
          config.serverUrl = resolvedOrigin;
          activeConfig.serverUrl = resolvedOrigin;

          const remote = remoteResponse.config;
    ...
    const widgetBundleCandidates = createServerUrlCandidates(config.serverUrl);
    let widgetBundleCode;

    try {
      const { data: bundleCode, origin } = await fetchFromCandidates(
        widgetBundleCandidates,
        '/widget-bundle.js',
        undefined,
        response => response.text()
      );
      widgetBundleCode = bundleCode;
      config.serverUrl = origin;
      activeConfig.serverUrl = origin;
    } catch (error) {
      logError('Failed to load widget bundle', error);
      console.error('[Chat Widget] Could not load from candidates:', widgetBundleCandidates.join(', '));
      return;
    }
    ...
    const messageHandlers = {
      resize(data) { iframe.style.width = `${data.width}px`; iframe.style.height = `${data.height}px`; },
      analytics(data) {
        if (!privacyPrefs.optedOut && typeof gtag !== 'undefined') {
          gtag('event', data.event, { event_category: 'Chat Widget', event_label: data.label, value: data.value });
        }
      },
      privacy(data) { const action = data.action && privacyActions[data.action]; if (action) action(data); },
      ready() { if (config.debug || window.ChatWidgetDebug) console.log('[Chat Widget] Widget ready'); },

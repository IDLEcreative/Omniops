...
const ChatRequestSchema = z.object({
  ...
  config: z.object({
    ...
    ai: z.object({
      maxSearchIterations: z.number().min(1).max(3).optional().default(2), // Reduced from 3
      searchTimeout: z.number().min(1000).max(10000).optional().default(5000), // Reduced from 10000
    }).optional(),
  }).optional(),
});
...
async function executeSmartSearch(
  ...
  try {
    ...
    // Add appropriate searches based on type
    if (searchType === "products" || searchType === "mixed") {
      // Try WooCommerce for product queries
      searchPromises.push(
    ...
    if (searchType === "information" || searchType === "mixed") {
      // Semantic search for information
      searchPromises.push(
        searchSimilarContent(query, browseDomain, Math.min(limit, 5), 0.15, timeoutMs - 500)
    ...
    const conversationMessages = [
      {
        ...
        content: `You are a helpful customer service assistant. Provide concise, accurate answers based on search results.
When searches return many results, summarize the most relevant ones rather than listing everything.

...
export async function POST(request: NextRequest) {
  try {
    ...
    // 1. Demo mode - now uses Crawlee/Playwright instead of Firecrawl
    if (demoId) {
      console.log('Demo mode available - using Crawlee/Playwright scraper');
    }
    ...
    // 4. WooCommerce product context (if enabled and configured)
    const isProductQuery = /product|item|buy|purchase|stock|available/i.test(message);
    
    if (domain && isProductQuery) {
      ...
      wooCommerceSearchPromise = (async () => {
        try {
          ...
          if (customerConfig?.woocommerce_enabled) {
            try {
              const { searchProductsDynamic } = await import('@/lib/woocommerce-dynamic');
              return await searchProductsDynamic(domain, message, 3);
            } catch (error) {
              console.error('WooCommerce search failed:', error);
              return [];
            }
          }
          return [];
    ...
    } else if (config?.features?.woocommerce?.enabled && isProductQuery) {
      // Fallback to environment variables if no domain (for testing)
      wooCommerceSearchPromise = import('@/lib/woocommerce')
        .then(async ({ searchProducts }) => {
          try {
            return await searchProducts(message, 3);
          } catch (error) {
            console.error('WooCommerce search failed:', error);
            return [];
          }
        ...
        .catch((error: any) => {
          console.error('WooCommerce import failed:', error);
          return [];
        });
      if (wooCommerceSearchPromise) {
        contextPromises.push(wooCommerceSearchPromise);
    ...
    if (embeddingSearchPromise) {
      ...
      if (embeddingResult && embeddingResult.status === 'fulfilled' && embeddingResult.value) {

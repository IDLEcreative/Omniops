...
class PriceParser {
     ...
     */
    static isContactPrice(priceString) {
     ...
     */
    static detectCurrency(priceString) {
     ...
     */
    static extractPriceValues(priceString) {
     ...
     */
    static isSalePrice(priceString) {
        const salePatterns = [
            /original\s*price/i,
            /was\s*[:£$€]/i,
            /current\s*price/i,
     ...
     */
    static parseVATPrice(priceString, prices, currency) {
     ...
     */
    static parseSimplePrice(price, currency, raw) {
        return {
            value: price,
     ...
     */
    static parseMultiplePrices(prices) {
        return prices
            .filter(p => p)
    ...
    static getBestPrice(prices) {
        ...
        if (salePrices.length > 0) {
            return salePrices.reduce((best, current) => (current.value || 0) < (best.value || Infinity) ? current : best);
        }
        // Return lowest regular price
        return validPrices.reduce((best, current) => (current.value || 0) < (best.value || Infinity) ? current : best);
    }
}
exports.PriceParser = PriceParser;
PriceParser.currencySymbols = {
    '£': 'GBP',
    '$': 'USD',
    '€': 'EUR',
    '¥': 'JPY',

...
export interface AbandonedCart {
  ...
  cart: { total: string; currency: string; itemCount: number; items: CartItem[] };
  dates: { created: string; modified: string; abandoned_duration: string };
  recovery: { priority: 'high' | 'medium' | 'low'; suggested_action: string; recovery_url?: string };
}
...
export class WooCommerceCartTracker {
  ...
  async getAbandonedCarts(options?: {
    ...
    const { limit = 20, includeStatuses = ['pending', 'on-hold', 'failed'], minValue = 0, hoursOld = 1 } = options || {};

    try {
      const orders = await this.wc.getOrders({ status: 'any' as const, per_page: limit, orderby: 'date', order: 'desc' });
      const now = new Date();
  ...
  async getAbandonedCart(orderId: number): Promise<AbandonedCart | null> {
    try {
      ...
      if (!['pending', 'on-hold', 'failed'].includes(order.status)) return null;
  ...
  async getCartRecoveryStats(days: number = 7): Promise<{
    ...
    try {
      ...
      const abandonedOrders = await this.wc.getOrders({ status: 'pending' as const, after: startDate.toISOString(), per_page: 100 });
      const completedOrders = await this.wc.getOrders({ status: 'completed' as const, after: startDate.toISOString(), per_page: 100 });
      ...
      stats.recovery_rate = totalCarts > 0 ? (completedOrders.length / totalCarts) * 100 : 0;
  ...
  async sendRecoveryReminder(orderId: number): Promise<{ success: boolean; message: string }> {
    try {
      const cart = await this.getAbandonedCart(orderId);
      if (!cart) return { success: false, message: 'Cart not found or already recovered' };
      await this.wc.createOrderNote(orderId, { note: `Cart recovery reminder scheduled for ${cart.customer.email}`, customer_note: false });
      return { success: true, message: `Recovery reminder marked for ${cart.customer.email}` };
    } catch (error) {
      console.error('Error sending recovery reminder:', error);
      return { success: false, message: 'Failed to send recovery reminder' };
    }
  }
  ...
  private transformOrderToCart(order: Order): AbandonedCart {
    ...
    const duration = hoursSinceCreated < 24 ? `${Math.round(hoursSinceCreated)} hours` : `${Math.round(hoursSinceCreated / 24)} days`;
  ...
  private calculatePriority(value: number, hoursOld: number): { priority: 'high' | 'medium' | 'low'; suggested_action: string } {
    if (value > 100 && hoursOld < 24) return { priority: 'high', suggested_action: 'Send immediate recovery email with discount code' };
    if (value > 50 || hoursOld < 48) return { priority: 'medium', suggested_action: 'Send friendly reminder email' };
    return { priority: 'low', suggested_action: 'Add to weekly recovery campaign' };
  }
}

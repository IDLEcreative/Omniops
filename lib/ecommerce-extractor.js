...
const product_normalizer_1 = require("./product-normalizer");
const pattern_learner_1 = require("./pattern-learner");
const scraper_config_1 = require("./scraper-config");
const price_parser_1 = require("./price-parser");
class EcommerceExtractor extends content_extractor_1.ContentExtractor {
    ...
    static async extractProductData($, url) {
            ...
            switch (method) {
                case 'learned-patterns':
                    if (config.extraction.strategies.patternLearningEnabled) {
                        const learnedProduct = await pattern_learner_1.PatternLearner.applyPatterns(url, $);
        ...
        if (rawProduct) {
            ...
            const normalizedProduct = product_normalizer_1.ProductNormalizer.normalizeProduct(rawProduct);
            // Learn from successful extraction
            if (normalizedProduct && normalizedProduct.name) {
                await pattern_learner_1.PatternLearner.learnFromExtraction(url, [normalizedProduct], {
    ...
    static async extractProductListing($, url, platform) {
        ...
        const normalizedProducts = products.map(p => product_normalizer_1.ProductNormalizer.normalizeProduct(p));
        // Learn from successful extraction if we found products
        if (normalizedProducts.length > 0) {
            await pattern_learner_1.PatternLearner.learnFromExtraction(url, normalizedProducts, {
                platform,

...
function calculateRetryPatterns(data: any[]): TelemetryStats['retryPatterns'] {
  if (data.length === 0) {
    return {
      avgRetries: 0,
      successRate: 0,
...
function calculateDomainLookupStats(data: any[]): TelemetryStats['domainLookup'] {
  if (data.length === 0) {
    return {
      methodDistribution: {},
      avgDuration: 0,
...
function calculateCircuitBreakerStats(data: any[]): TelemetryStats['circuitBreaker'] {

...
class AdaptiveEntityExtractor {
     ...
     */
    async initializeForDomain(domainId) {
        ...
        const classification = await business_classifier_1.BusinessClassifier.classifyBusiness(domainId, sampleContent, pages[0] ? pages[0].metadata : undefined);
    ...
    async extractEntities(pageId) {
        ...
        const extractionPrompt = this.buildExtractionPrompt(page, this.businessClassification);
     ...
     * Build extraction prompt based on business type
     */
    buildExtractionPrompt(page, classification) {
...
Return ONLY valid JSON matching the structure above.`;
     ...
     */
    async storeEntity(page, extracted, classification) {
        const schema = classification.suggestedSchema;
        const terminology = classification.terminology;
     ...
     */
    parseAvailability(value, classification) {
     ...
     */
    async processDomainsEntities(domainId, limit = 50) {

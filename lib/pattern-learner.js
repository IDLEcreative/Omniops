...
class PatternLearner {
     ...
     */
    static async learnFromExtraction(url, products, extractionData) {
        try {
            ...
            if (products.length > 0) {
                ...
                if (sampleProduct && sampleProduct.price) {
                    patterns.push({
                        ...
                        extractionMethod: extractionData.extractionMethod || 'dom'
                    });
                }
                // Learn name patterns
     ...
     */
    static async saveDomainPatterns(domain, patterns) {
        ...
        if (existing) {
            ...
            const mergedPatterns = this.mergePatterns(existing.patterns || [], patterns.patterns);
     ...
     */
    static async applyPatterns(url, $ // CheerioAPI
    ) {
        try {
     ...
     */
    static async updatePatternSuccess(url, success, usedPatterns) {
        const domain = new URL(url).hostname;
        const patterns = await this.getPatterns(url);
        if (!patterns)
            return;
     ...
     */
    static async getRecommendations(url, platform) {
        if (!platform)
            return [];

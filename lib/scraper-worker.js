...
async function splitIntoChunks(text, maxChunkSize = 1000, pageUrl = '', htmlContent = null) {
  ...
  // Note: ContentDeduplicator works on full page content, not chunks
  // Chunk deduplication is handled during embedding generation
  // So we just return the non-boilerplate chunks directly
...
async function generateEmbeddings(chunks) {
  // Note: ContentDeduplicator is for full pages, not chunks
  // Generate embeddings for all chunks directly
              ...
              if (pageData.content && pageData.content.length > 0) {
                ...
                if (!existingEmbeddings || existingEmbeddings.length === 0 || FORCE_RESCRAPE) {
                  ...
                  if (chunks.length > 0) {
                    ...
                    // Insert embeddings directly to database
                    // Note: DatabaseOptimizer doesn't have bulkInsertEmbeddings, so we use direct insert
          ...
          if (domainId) {
            const { error: updateError } = await supabase
              ...
              .upsert({
                ...
                onConflict: 'domain_id,url', // Specify unique constraint columns

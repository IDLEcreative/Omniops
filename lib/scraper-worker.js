...
// const { MetadataExtractor } = require('./metadata-extractor'); // Disabled - TS file
...
// Import optimized semantic chunker
// const { SemanticChunkerOptimized: SemanticChunker } = require('./semantic-chunker-optimized'); // Disabled - TS file
            ...
            if (pageError) {
              ...
              if (pageData.content && pageData.content.length > 0) {
                ...
                if (!existingEmbeddings || existingEmbeddings.length === 0 || FORCE_RESCRAPE) {
                  ...
                  const chunks = await splitIntoChunks(pageData.content, 1000, pageUrl, html);
                  ...
                  if (chunks.length > 0) {
                    ...
                    // Store new embeddings with enhanced metadata
                    const embeddingRecords = await Promise.all(chunks.map(async (chunk, index) => {
                      // Extract enhanced metadata for each chunk
                      // Temporarily disabled - MetadataExtractor is a TS file
                      const enhancedMetadata = {
                        chunk_type: 'paragraph',
                        semantic_density: 0.5,
                        position_weight: 1 - (index / chunks.length),
                        has_keywords: true,

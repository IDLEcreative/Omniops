  ...
  static smartChunk(text, options = {}) {
    ...
    for (const sentence of sentences) {
      ...
      if (potentialChunk.length > maxChunkSize && currentChunk) {
        ...
        chunks.push(trimmedSentence.substring(0, maxChunkSize).trim());
        currentChunk = '';
      } else {
    ...
    // Final validation: ensure no chunk exceeds maxChunkSize
    return chunks
      .filter(chunk => chunk.length > 100)
      .map(chunk => chunk.substring(0, maxChunkSize));
  }
}
...
function simpleChunking(text, maxChunkSize) {
  ...
  for (const sentence of sentences) {
    ...
    if (potentialChunk.length > maxChunkSize && currentChunk) {
      // Save current chunk (enforcing max size) and start new one
      chunks.push(currentChunk.substring(0, maxChunkSize).trim());
      currentChunk = trimmedSentence;
    } else if (trimmedSentence.length > maxChunkSize) {
      // Single sentence exceeds max size, split it
            ...
            if (pageError) {
              ...
              if (pageData.textContent && pageData.textContent.length > 0) {
                ...
                if (!existingEmbeddings || existingEmbeddings.length === 0 || FORCE_RESCRAPE) {
                  ...
                  // OPTIMAL: 1500 chars balances semantic coherence with search precision
                  const chunks = await splitIntoChunks(enrichedContent, 1500, pageUrl, html);

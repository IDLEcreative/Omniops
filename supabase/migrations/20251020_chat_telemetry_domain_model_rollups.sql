-- Extend telemetry rollups with domain/model aggregations for dashboard performance.

-- Create rollup tables if they do not exist
CREATE TABLE IF NOT EXISTS public.chat_telemetry_domain_rollups (
  ...
  CONSTRAINT chat_telemetry_domain_rollups_pkey PRIMARY KEY (granularity, bucket_start, domain)
);

CREATE TABLE IF NOT EXISTS public.chat_telemetry_model_rollups (
  bucket_start TIMESTAMPTZ NOT NULL,
  bucket_end TIMESTAMPTZ NOT NULL,
  granularity TEXT NOT NULL CHECK (granularity IN ('hour', 'day')),
  domain TEXT,
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  CONSTRAINT chat_telemetry_model_rollups_pkey PRIMARY KEY (granularity, bucket_start, COALESCE(domain, ''), model)
);

COMMENT ON TABLE public.chat_telemetry_domain_rollups IS 'Aggregated telemetry metrics per domain for faster dashboard breakdowns.';
COMMENT ON TABLE public.chat_telemetry_model_rollups IS 'Aggregated telemetry metrics per domain/model pair to support detailed dashboards.';

-- Indexes for fast lookups by domain/model
CREATE INDEX IF NOT EXISTS idx_chat_telemetry_domain_rollups_domain ON public.chat_telemetry_domain_rollups(domain, bucket_start);
CREATE INDEX IF NOT EXISTS idx_chat_telemetry_model_rollups_domain ON public.chat_telemetry_model_rollups(domain, bucket_start);
CREATE INDEX IF NOT EXISTS idx_chat_telemetry_model_rollups_model ON public.chat_telemetry_model_rollups(model, bucket_start);

-- Replace refresh function so it populates the new tables
...
BEGIN
  ...
  -- Retention windows (hourly -> 90 days, daily -> 400 days)
...
'Aggregates chat telemetry into hour/day buckets plus per-domain/model rollups for dashboard performance.';

GRANT EXECUTE ON FUNCTION public.refresh_chat_telemetry_rollups(TEXT, TIMESTAMPTZ) TO service_role;

-- Initial backfill to ensure new tables are populated
